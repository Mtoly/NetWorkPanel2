...
const tableData = computed(() => {
  let ret: any = []
  let index = 0
  const periodData = showPrev.value ? rankingStore.rankData.prev : rankingStore.rankData.now
  ;([3, 2, 1, 0]).forEach((i: number) => {
    if (showRank.value) {
      ret.push({
        des: desMap[showPrev.value ? 1 : 0][index],
        allUsed: periodData[i]?.["allUsed"]?.[0] || 0,
        averageSpeed: periodData[i]?.["averageSpeed"]?.[0] || 0,
        onlineTime: periodData[i]?.["onlineTime"]?.[0] || 0,
      })
    } else {
      ret.push({
        des: desMap[showPrev.value ? 1 : 0][index],
        allUsed: formatter(periodData[i]?.["allUsed"]?.[1] || 0, 0),
        averageSpeed: formatter((periodData[i]?.["averageSpeed"]?.[1] || 0) * 8, 1),
        onlineTime: timeFormatter(periodData[i]?.["onlineTime"]?.[1] || 0),
      })
    }
    index++
  })
...
const login = async () => {
  imgBase64.value = ''
  let resp = await rankingApi.thirdQQLogin(rankingStore.accessToken)
  rankingStore.setLoginStatus(-1)
  if (resp.status == 0) {
    imgBase64.value = resp.img || ''
    thirdQQLoginUrl.value = resp.url || ''
    rankingStore.setSession(resp.session || '')
    qr_check()
  }
}

const kick_old = async () => {
  ElMessageBox.confirm('确定要强制下线登录该账号的其他设备吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(async () => {
    let resp = await rankingApi.kickOld(rankingStore.accessToken)
    if (resp.status == 0) {
      ElMessage({ type: 'info', message: `已经下线其他设备` })
    }
  }).catch(() => {})
}

const logOut = async () => {
  ElMessageBox.confirm('确定从此设备退出登录吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning',
  }).then(() => {
...
const bindQQFunc = async () => {
  let resp = await rankingApi.thirdQQBind(bindQQ.value, rankingStore.getSession())
  if (resp.status == 0) {
    ElMessage.success("绑定成功")
    qr_check()
  } else if (resp.status == -3 || resp.status == -1) {
    ElMessageBox.alert(`${resp.msg}`, '提示', { confirmButtonText: '确定' })
  } else {
...
const qr_check = async (once: boolean = false): Promise<boolean> => {
  let resp = await rankingApi.thirdQQCheck(rankingStore.getSession())
  if (resp.status == 0) {
    rankingStore.setLoginStatus(resp.uin || 0)
    rankingStore.setAccessToken(resp.AccessToken || '')
    rankingStore.clearSession()
    getStatus()
    return true
  } else if (resp.status == -1) {
    rankingStore.setLoginStatus(-1)
    if (isMounted && !once) {
      setTimeout(() => { qr_check() }, 1000)
    } else {
      rankingStore.clearSession()
    }
  } else if (resp.status == -2) {
    rankingStore.setLoginStatus(-2)
    return true
  } else {
    rankingStore.clearSession()
...
const getStatus = async () => {
  let resp = await rankingApi.getStatus(rankingStore.accessToken)
  if (resp.status == 0) {
    rankingStore.setLoginStatus(resp.uin || 0)
    rankingStore.setUserInfo(resp.uin || 0, resp.nick || '', resp.data || { now: {}, prev: {} })
  } else if (resp.status == -1) {
    if (rankingStore.getSession()) {
...
<template>
  <div class="block">
    <div v-loading="!rankingStore.loginStatus" class="card">
      <div v-if="rankingStore.loginStatus==-1">
      ...
      <div v-if="rankingStore.loginStatus==-2">
      ...
      <div v-if="rankingStore.loginStatus>0">
        <el-avatar :size="100" :src="'https://q.qlogo.cn/headimg_dl?dst_uin='+rankingStore.uin+'&spec=640'" />
        <br>
        <span style="font-size: 20px;">{{rankingStore.nick}}</span>
        <br>
